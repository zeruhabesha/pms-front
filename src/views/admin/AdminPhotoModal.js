...
import { uploadAdminPhoto } from '../../api/actions/AdminActions';

const AdminPhotoModal = ({ visible, setVisible, editingAdmin, handleSave }) => {
  ...
  const handlePhotoSave = async () => {
    if (editingAdmin && selectedPhoto) {
      try {
        const formData = new FormData();
        formData.append('photo', selectedPhoto);

        await dispatch(uploadAdminPhoto({ id: editingAdmin._id, photo: formData })).unwrap();
        handleSave();
        setVisible(false);
      } catch (error) {
...
export default AdminPhotoModal;